...
export async function PUT(
  ...
  try {
    ...
    const existing = db.prepare("SELECT * FROM Bet WHERE id = ?").get(Number(id)) as
      { betAmount: number; odds: number; result: string; userId: number } | undefined;
    ...
    const { betAmount, odds: winAmount, userId } = existing;
...
export async function DELETE(
  ...
  try {
    ...
    // odds 字段存储 winAmount（可赢金额）
    const { betAmount, odds: winAmount, result, userId } = existing;
